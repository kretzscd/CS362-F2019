...
void minion_actionAttack(int choice1, int choice2, struct gameState *state,\
...
int ambassador_actionAttack(int choice1, int choice2, struct gameState *state,\
        int handPos, int currentPlayer)
{
        int i, j;
...
void tribute_action(struct gameState *state, int currentPlayer, int nextPlayer,\
        int *tributeRevealedCards)
{
        int i;
        if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
            if (state->deckCount[nextPlayer] > 0) {
                tributeRevealedCards[0] = state->deck[nextPlayer]\
...
int mine_action(int choice1, int choice2, struct gameState *state, int handPos,\
        int currentPlayer)
{
    // declare two int variables used in function; i will be used in for loop
    int i, j;

    // store card we will trash
    j = state->hand[currentPlayer][choice1]; 

    // check to see if the card the player wants to discard is a Treasure card 
    ...
    // check to see if player's choice of new Treasure card is <= 3
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        // call the mine_action function which will perform the card's action
        // the function returns an int that will be used for the return
        return(mine_action(choice1, choice2, state, handPos, currentPlayer));
    ...
    case minion:
        //call function to perform minion action - attack
        minion_actionAttack(choice1, choice2, state, handPos, currentPlayer);
        return 0;
    ...
    case tribute:
        //call function to perform tribute action
        tribute_action(state, currentPlayer,  nextPlayer,  tributeRevealedCards);
        return 0;

    case ambassador:
        //call function to perform ambassador's action-attack
        //it will return an int that is used by the return
        return(ambassador_actionAttack( choice1, choice2, state, handPos,\
                    currentPlayer));
